...
class EAF(object):
    ...
    def _ordered_buffer_view_keys(self, buffer_id):
        ordered_keys = []

        if self.views_data:
            for view_info in self.views_data.split(","):
                if not view_info:
                    continue
                if view_info.split(":")[0] != buffer_id:
                    continue
                view_key = self.view_identity(view_info)
                if view_key in self.view_dict and view_key not in ordered_keys:
                    ordered_keys.append(view_key)

        for key, view in self.view_dict.items():
            if view.buffer_id == buffer_id and key not in ordered_keys:
                ordered_keys.append(key)

        return ordered_keys

    def _pixmap_looks_placeholder(self, pixmap):
        if pixmap.isNull():
            return True
        ...
        samples = []
        x_steps = max(2, min(5, width))
        y_steps = max(2, min(5, height))
        for y_index in range(y_steps):
            y = min(height - 1, int((height - 1) * y_index / max(1, y_steps - 1)))
            for x_index in range(x_steps):
                x = min(width - 1, int((width - 1) * x_index / max(1, x_steps - 1)))
        ...
        avg = [sum(sample[channel] for sample in samples) / len(samples) for channel in range(3)]
        grayish = max(abs(avg[0] - avg[1]), abs(avg[1] - avg[2]), abs(avg[0] - avg[2])) < 6
        bright = sum(avg) / 3 > 220

        return channel_spread < 8 and grayish and bright

    def save_buffer_views_screenshots_sync(self, buffer_id, output_dir, shot_names):
        return self._save_buffer_views_screenshots(buffer_id, output_dir, shot_names)

    @PostGui()
    def _save_buffer_views_screenshots(self, buffer_id, output_dir, shot_names):
        os.makedirs(output_dir, exist_ok=True)
        names = [name for name in shot_names.split(",") if name]
        ordered_keys = self._ordered_buffer_view_keys(buffer_id)
        saved = 0

        key_index = 0
        for shot_name in names:
            view_key = None
            pixmap = None
            while key_index < len(ordered_keys):
                candidate_key = ordered_keys[key_index]
                key_index += 1
                view = self.view_dict.get(candidate_key)
                if view is None:
                    continue
                candidate_pixmap = view.screen_shot()
                if self._pixmap_looks_placeholder(candidate_pixmap) and key_index < len(ordered_keys):
                    continue
                view_key = candidate_key
                pixmap = candidate_pixmap
                break

            if view_key is None or pixmap is None:
                continue

            image_path = os.path.join(output_dir, shot_name + ".png")
            pixmap.save(image_path)
            saved += 1

        return saved

    def probe_buffer_views_screenshots_sync(self, buffer_id, output_dir, shot_names):
        return self._probe_buffer_views_screenshots(buffer_id, output_dir, shot_names)

    @PostGui()
    def _probe_buffer_views_screenshots(self, buffer_id, output_dir, shot_names):
        os.makedirs(output_dir, exist_ok=True)
        names = [name for name in shot_names.split(",") if name]
        ordered_keys = self._ordered_buffer_view_keys(buffer_id)
        statuses = []
        ...
        for shot_name in names:
            ...
            while key_index < len(ordered_keys):
                ...
                if view is None:
                    continue
                candidate_pixmap = view.screen_shot()
                ready = not self._pixmap_looks_placeholder(candidate_pixmap)
                if (not ready) and key_index < len(ordered_keys):
                    continue
                view_key = candidate_key
                pixmap = candidate_pixmap
                break
            ...
            image_path = os.path.join(output_dir, shot_name + ".png")
            pixmap.save(image_path)
            statuses.append("1" if ready else "0")
...
def eaf_start_embedded_thread(width: int, height: int) -> None:
    ...
    def _run():
        ...
        is_mac_port = bool(get_emacs_func_cache_result("eaf--mac-port-p", []))
        ...
        # Browser/App init on mac-port cannot safely block the AppKit/Lisp
        # threads on arbitrary sync bridge lookups, so keep the broad prewarm
        # here and avoid lazy bridge misses during first app startup.
        if is_mac_port:
            get_emacs_vars([
                "eaf-config-location",
