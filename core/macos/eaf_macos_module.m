...
static pthread_mutex_t gViewUpdateMu = PTHREAD_MUTEX_INITIALIZER;
static NSString *gPendingViewSpecs = nil;
static BOOL gViewUpdateQueued = NO;
...
static emacs_value Fframe_window_number(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data);
static emacs_value Fbridge_pending_p(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data);
...
static BOOL bridge_queue_has_pending_items(void) {
    BOOL pending = NO;
    pthread_mutex_lock(&gBridgeMu);
    pending = (gBridgeHead != gBridgeTail);
...
/// (eaf-macos-bridge-pending-p) → t / nil
static emacs_value
Fbridge_pending_p(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data) {
    return env->intern(env, bridge_queue_has_pending_items() ? "t" : "nil");
}
...
static NSWindow *windowForFrameHandle(uintptr_t frameHandle) {
    if (frameHandle == 0) {
        return nil;
    }

    id object = (__bridge id)((void *)frameHandle);
    if (!object) {
        return nil;
    }
    ...
    if ([object respondsToSelector:@selector(emacsWindow)]) {
        id maybeWindow = ((id (*)(id, SEL))objc_msgSend)(object, @selector(emacsWindow));
        if ([maybeWindow isKindOfClass:[NSWindow class]]) {
            return (NSWindow *)maybeWindow;
        }
    }

    if ([object respondsToSelector:@selector(emacsView)]) {
        id maybeView = ((id (*)(id, SEL))objc_msgSend)(object, @selector(emacsView));
        if ([maybeView isKindOfClass:[NSView class]]) {
            NSWindow *window = [(NSView *)maybeView window];
            if (window) {
                return window;
            }
        }
    ...
    if ([object respondsToSelector:@selector(window)]) {
        id maybeWindow = ((id (*)(id, SEL))objc_msgSend)(object, @selector(window));
        if ([maybeWindow isKindOfClass:[NSWindow class]]) {
            return (NSWindow *)maybeWindow;
        }
    }
...
static NSString *describeWindow(NSWindow *window) {
    if (!window) {
        return @"window=nil";
    }

    return [NSString stringWithFormat:@"window=%p number=%ld title=%@ key=%d main=%d visible=%d",
            window,
...
static NSString *describeFrameHandle(uintptr_t frameHandle) {
    if (frameHandle == 0) {
        return @"handle=0";
    }

    id object = (__bridge id)((void *)frameHandle);
    NSString *className = object ? NSStringFromClass([object class]) : @"nil";
    NSWindow *window = windowForFrameHandle(frameHandle);
    return [NSString stringWithFormat:@"handle=0x%llx class=%@ %@",
            (unsigned long long)frameHandle,
            className,
...
static void activateEmacsWindowForBufferId(NSString *bufferId) {
    runOnAppKitAsync(^{
        NSWindow *window = nil;
        BOOL hasBufferTarget = bufferId.length > 0;

        if (hasBufferTarget) {
            window = windowForBufferId(bufferId);
        } else {
            window = emacsWindow();
        }
        if (!window) {
...
static void processViewSpecs(NSString *viewSpecs) {
...
static void drainPendingViewUpdates(void) {
    for (;;) {
        NSString *viewSpecs = nil;

        pthread_mutex_lock(&gViewUpdateMu);
        if (gPendingViewSpecs) {
            viewSpecs = [gPendingViewSpecs copy];
            gPendingViewSpecs = nil;
        } else {
            gViewUpdateQueued = NO;
        }
        pthread_mutex_unlock(&gViewUpdateMu);

        if (!viewSpecs) {
            break;
        }

        processViewSpecs(viewSpecs);
    }
}
...
/// BUFFER-ID<TAB>VIEW-ID<TAB>FRAME-HANDLE<TAB>FRAME-W<TAB>FRAME-H<TAB>LOCAL-X<TAB>LOCAL-Y<TAB>LOCAL-W<TAB>LOCAL-H
static emacs_value
Fupdate_views(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data) {
    if (nargs != 1) return env->intern(env, "nil");

    NSString *viewSpecs = [extractNSString(env, args[0]) copy];
    BOOL shouldQueue = NO;

    pthread_mutex_lock(&gViewUpdateMu);
    gPendingViewSpecs = viewSpecs;
    if (!gViewUpdateQueued) {
        gViewUpdateQueued = YES;
        shouldQueue = YES;
    }
    pthread_mutex_unlock(&gViewUpdateMu);
...
/// (eaf-macos-frame-window-number FRAME-HANDLE) → integer
static emacs_value
Fframe_window_number(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data) {
    if (nargs != 1) return env->make_integer(env, 0);

    NSString *handleString = extractNSString(env, args[0]);
    uintptr_t handle = (uintptr_t)handleString.longLongValue;
    __block NSInteger windowNumber = 0;

    runOnMainSync(^{
        NSWindow *window = windowForFrameHandle(handle);
        if (window) {
            windowNumber = window.windowNumber;
        }
    });

    return env->make_integer(env, (intmax_t)windowNumber);
}

/// (eaf-macos-debug-frame-handle HANDLE) → string
static emacs_value
Fdebug_frame_handle(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data) {
    if (nargs != 1) return env->intern(env, "nil");
    ...
    runOnMainSync(^{
        result = [describeFrameHandle(handle) copy];
    });

    if (!result) {
        result = @"";
    }
    return env->make_string(env, result.UTF8String, result.length);
}

/// (eaf-macos-debug-view-registry) → string
static emacs_value
Fdebug_view_registry(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data) {
    __block NSString *result = nil;
    runOnMainSync(^{
        NSMutableArray<NSString *> *lines = [NSMutableArray array];
        for (NSString *key in registry()) {
            EAFViewState *state = registry()[key];
            [lines addObject:[NSString stringWithFormat:@"view=%@ buffer=%@ hostView=%p container=%p %@",
                              key,
                              state.bufferId ?: @"",
                              state.hostView,
                              state.containerView,
                              describeWindow(state.hostWindow ?: state.containerView.window ?: state.hostView.window)]];
        }
...
int emacs_module_init(struct emacs_runtime *ert) {
    ...
    bindFunction(env, "eaf-macos-bridge-pending-p", 0, 0, Fbridge_pending_p,
                 "Return t when the embedded macOS bridge queue has pending work.");
    ...
    bindFunction(env, "eaf-macos-frame-window-number", 1, 1, Fframe_window_number,
                 "Resolve an Emacs frame handle to a stable native NSWindow windowNumber.");

    bindFunction(env, "eaf-macos-debug-frame-handle", 1, 1, Fdebug_frame_handle,
                 "Return a debug description for a frame handle passed from Emacs.");

    bindFunction(env, "eaf-macos-debug-view-registry", 0, 0, Fdebug_view_registry,
                 "Return a debug dump of native macOS EAF view bindings.");
